...
(function() {
    ...
    if (mZoneLeft) {
        mZoneLeft.addEventListener('touchstart', (e) => {
            e.preventDefault();
            // Lấy ngón tay đầu tiên chạm vào vùng này
        ...
        mZoneLeft.addEventListener('touchmove', (e) => {
            ...
            // Tìm đúng ngón tay đang di chuyển JoyStick
            let touch = Array.from(e.touches).find(t => t.identifier === joyActiveId);
            if (!touch) return;
            ...
            // Bơm lệnh giả lập WASD cho Engine
            let threshold = 15; 
        ...
        function resetJoyStick(e) {
            // Chỉ reset nếu ngón tay nhấc lên đúng là ngón JoyStick
            let isJoyFingerLifted = Array.from(e.changedTouches).some(t => t.identifier === joyActiveId);
            if (isJoyFingerLifted || e.type === 'touchcancel') {
                joyActiveId = null;
    ...
    if (btnQ) {
        btnQ.addEventListener('touchstart', (e) => {
            e.preventDefault();
            let touch = e.changedTouches[0];
            qActiveId = touch.identifier;
            isDraggingQ = false;
            qStartY = touch.clientY;
            btnQ.style.transform = 'scale(0.9)'; 
        }, {passive: false});

        btnQ.addEventListener('touchmove', (e) => {
            e.preventDefault();
            if (qActiveId === null) return;
            let touch = Array.from(e.touches).find(t => t.identifier === qActiveId);
            if (!touch) return;
            
            let dy = touch.clientY - qStartY;
            if (Math.abs(dy) > 20) { 
                isDraggingQ = true;
                if (dy < -20) { window.keys.space = true; window.keys.shift = false; } // Kéo lên -> Bay
                else if (dy > 20) { window.keys.shift = true; window.keys.space = false; } // Kéo xuống -> Hạ
            } else {
                window.keys.space = false; window.keys.shift = false;
            }
        }, {passive: false});

        function resetBtnQ(e) {
            let isQFingerLifted = Array.from(e.changedTouches).some(t => t.identifier === qActiveId);
            if (isQFingerLifted || e.type === 'touchcancel') {
                qActiveId = null;
                btnQ.style.transform = 'scale(1)';
                window.keys.space = false; window.keys.shift = false;
                if (!isDraggingQ) kichHoatKyNang('Q'); // Chỉ TAP mới xuất chiêu
            }
        }
