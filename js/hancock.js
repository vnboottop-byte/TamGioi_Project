...
// 👑 CÔNG NGHỆ: TRÁI TIM ĐẠI BÁC + MƯA TÊN KHÔNG GIAN + BỤI LỬA HỒNG
// ==========================================
...
(function () {
    ...
    // 🌟 3. HIỆU ỨNG NỔ: VỤ NỔ HỒNG & ÂM THANH
    function taoHieuUngNoBoa(pos, isBig = false) {
        ...
        if (!window.textureBuiLuaBoa) {
            ...
            gradient.addColorStop(0.2, 'rgba(255, 102, 178, 1)'); // 💖 MÀU HỒNG ĐẶC TRƯNG HANCOCK
    ...
    // 🌟 4. TÀN LỬA HỒNG BAY THEO MŨI TÊN (SAO BĂNG BOA)
    ...
    // 🌟 5. ĐÚC MODEL DÙNG CHUNG (TIM & ARROW)
    ...
    window.thoiDiemChemCuoi_Boa = window.thoiDiemChemCuoi_Boa || 0;

    // ==========================================
    // 🏹 TUNG CHIÊU BOA HANCOCK
    // ==========================================
    window.tungComboBoa = function (phim, isRemote = false, remoteGoc = null, remoteDich = null, remoteHuong = null, casterId = null, weaponUrl = null) {
        ...
        if (isRemote === false) {
            let bayGio = Date.now();
            if (bayGio - window.thoiDiemChemCuoi_Boa < 800) return;
            window.thoiDiemChemCuoi_Boa = bayGio;
        ...
        // ===============================================
        // 💖 CHIÊU Q (ATTACK1): 3 TIM BẮN THẲNG
        // ===============================================
        if (phim === 'Q') {
            for (let i = 0; i < 3; i++) {
                setTimeout(() => {
                    const tim = taoVatTheBoa('TIM', 6); // Trái tim nhỏ vừa
                    let diemBan = viTriGocToTam.clone().add(huongMat.clone().multiplyScalar(2));
                    
                    tim.position.copy(diemBan);
                    tim.lookAt(mucTieu);
                    scene.add(tim);

                    kyNangBoa.push({
                        mesh: tim, type: 'BAY_THANG', speed: 6.0, life: 100, 
                        skillId: 'Q', targetPos: mucTieu.clone(), 
                        damage: dameGoc * 0.133, isRemote: isRemote, noBanKinh: 12 // DPS: 3 * 0.133 * 10 = 4.0
                    });
                }, i * 200); // Bắn liên tiếp 3 nốt
            }
        }

        // ===============================================
        // 💖 CHIÊU E (ATTACK2): 1 TIM KHỔNG LỒ ĐẬP TỪ TRÊN XUỐNG (BẺ TRỤC NẰM NGANG)
        // ===============================================
        else if (phim === 'E') {
            setTimeout(() => {
                const timTo = taoVatTheBoa('TIM', 35); // Trái tim khổng lồ
                
                let diemTha = diemChanMucTieu.clone();
                diemTha.y += 25.0; // Xuất hiện tít trên cao

                timTo.position.copy(diemTha);
                // 🌟 BÍ THUẬT: Bẻ trục X 90 độ để trái tim nằm bẹp ngang đập xuống!
                timTo.rotation.set(Math.PI / 2, 0, 0); 
                scene.add(timTo);

                kyNangBoa.push({
                    mesh: timTo, type: 'ROI_THANG_XUONG', speed: 1.0, life: 150, 
                    skillId: 'E', targetPos: diemChanMucTieu, 
                    damage: dameGoc * 0.6, isRemote: isRemote, noBanKinh: 30 // DPS: 0.6 * 3 = 1.8
                });
            }, 600); // Đợi diễn hoạt vung tay
        }

        // ===============================================
        // 💖 CHIÊU R (ATTACK5): VÒNG TRÒN MŨI TÊN (Copy Cung Thủ Q)
        // ===============================================
        else if (phim === 'R') {
            ...
            for (let i = 0; i < 8; i++) {
                const ten = taoVatTheBoa('boaarow', 6.0); // Mũi tên rắn hồng
                ...
                kyNangBoa.push({
                    ...
                    life: 200, ticks: 0, targetPos: mucTieu.clone(), 
                    damage: dameGoc * 0.0625, speed: 0.5, fireDelay: i * 8, isRemote: isRemote // DPS: 8 * 0.0625 * 2 = 1.0
                });
            }
        }

        // ===============================================
        // 💖 CHIÊU F (ATTACK4): MƯA TÊN KHÔNG GIAN (Copy Cung Thủ E)
        // ===============================================
        else if (phim === 'F') {
            const soLuong = 15;
            const spawnCenter = viTriGocToTam.clone().add(upVector.clone().multiplyScalar(25)).sub(huongMat.clone().multiplyScalar(5));
            ...
            for (let i = 0; i < soLuong; i++) {
                ...
                kyNangBoa.push({
                    ...
                    damage: dameGoc * 0.066, arcHeight: 30 + Math.random() * 20, fireDelay: i * 3, progress: 0, isRemote: isRemote,
                    upVector: upVector.clone() // DPS: 15 * 0.066 * 1 = 1.0
                });
            }
        }
    };
    ...
    window.updateCombatBoa = function () {
        for (let i = kyNangBoa.length - 1; i >= 0; i--) {
            ...
            // 2. VẬT LÝ TIM ĐẬP TỪ TRÊN TRỜI RƠI XUỐNG (CHIÊU E)
            else if (s.type === 'ROI_THANG_XUONG') {
                s.speed *= 1.1; // Rơi gia tốc trọng trường
            ...
            // 3. VẬT LÝ VÒNG TRÒN MŨI TÊN (CHIÊU R)
        ...
        for (let i = hieuUngBoa.length - 1; i >= 0; i--) {
            ...
            if (h.type === 'trail') {
                ...
                for (let j = 0; j < posArr.length / 3; j++) {
                    posArr[j * 3] += h.velocities[j].x; posArr[j * 3 + 1] += h.velocities[j].y; posArr[j * 3 + 2] += h.velocities[j].z;
                    h.velocities[j].x *= 0.85; h.velocities[j].z *= 0.85;
                    h.velocities[j].y *= 0.85; h.velocities[j].y += 0.05; // Bốc lên nhẹ
                }
                h.system.geometry.attributes.position.needsUpdate = true;
    ...
    if (typeof window.SCRIPT_PHAI_CUA_TOI !== 'undefined' && window.SCRIPT_PHAI_CUA_TOI.trim() !== '') {
        window.HePhaiHienTai = {
            ...
            khoiTao: function () {
                console.log("💖 Đã bị mê hoặc bởi Boa Hancock! Kích hoạt hệ thống màu hồng!");
