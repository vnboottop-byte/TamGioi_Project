...
(function () {
    const kyNangSmoker = [];
    const hieuUngSmoker = [];
    const danhSachSoBaySmoker = [];

    window.KHO_ANIM_NHANROI = [];
    window.KHO_ANIM_TANCONG = [];
    window.tongSoChuNoi_Smoker = 0;

    // ==========================================
    // 1. LÕI TIỆN ÍCH CƠ BẢN
    // ==========================================
    function taoSoSatThuongSmoker(pos3D, satThuong, mauSac = '#ffffff') {
        if (window.isMobile) return;
        if (satThuong <= 0 || (window.isMobile && window.tongSoChuNoi_Smoker > 5)) return;
        window.tongSoChuNoi_Smoker++;

        const div = document.createElement('div');
        div.innerText = "-" + Math.round(satThuong);
        let bongChu = window.isMobile ? '1px 1px 0px #000' : '0px 0px 10px #000, 2px 2px 0px #555555';
        div.style.cssText = `position:absolute; color:${mauSac}; font-weight:900; font-size:35px; text-shadow:${bongChu}; pointer-events:none; z-index:9999;`;
        document.body.appendChild(div);
        danhSachSoBaySmoker.push({ el: div, pos: pos3D.clone(), life: 60, offsetY: 0 });
    }

    window.layMucTieuGanNhatSmoker = function (viTriGoc) {
    ...
    function gaySatThuongSmoker(tamNo, luongSatThuong, banKinh) {
            ...
            for (let id in remotePlayers) {
                ...
                if (rp.status === 'ready' && rp.mesh && !laBoss && !mucTieuDaXyLy.has(rp.mesh)) {
                    ...
                    if (tamNo.distanceTo(hit.tamNguc) <= (banKinh + hit.banKinh)) {
                        ...
                        taoSoSatThuongSmoker(posHienSo, luongSatThuong, '#ffffff');
            ...
            window.danhSachQuaiVat.forEach(quai => {
                if (!quai.isDead && quai.mesh && !mucTieuDaXyLy.has(quai.mesh)) {
                    ...
                    if (tamNo.distanceTo(hit.tamNguc) <= (banKinh + hit.banKinh)) {
                        ...
                        if (quai.isBoss) {
                            taoSoSatThuongSmoker(hit.tamNguc.clone().add(new THREE.Vector3(0, 5, 0)), luongSatThuong, '#ffaa00');
                            if (typeof window.chemTrungBoss === 'function') window.chemTrungBoss(quai.id, luongSatThuong);
                        } else {
                            quai.hp -= luongSatThuong; taoSoSatThuongSmoker(hit.tamNguc.clone(), luongSatThuong);
    ...
    // ==========================================
    // 2. VỤ NỔ VÀ KHÓI MODEL
    // ==========================================
    function taoHieuUngNoKhoi(pos, isBig = false, upVector = new THREE.Vector3(0, 1, 0)) {
        ...
        if (!window.textureKhoi) {
            ...
            gradient.addColorStop(0.4, 'rgba(220, 220, 220, 0.9)');
            gradient.addColorStop(1, 'rgba(0, 0, 0, 0)');         
            ctx.fillStyle = gradient; ctx.fillRect(0, 0, 64, 64);
            window.textureKhoi = new THREE.CanvasTexture(canvas);
        }

        const mat = new THREE.PointsMaterial({
            color: 0xffffff, size: window.isMobile ? 5.0 : 12.0, map: window.textureKhoi, 
            transparent: true, opacity: 0.8, blending: THREE.NormalBlending, depthWrite: false
        });

        const pts = new THREE.Points(geo, mat); scene.add(pts);
        hieuUngSmoker.push({ system: pts, velocities: vels, life: 35, upVector: upVector.clone() }); 
    }

    function taoDuoiKhoi(pos, dirNguoc, speed) {
        ...
        geo.setAttribute('position', new THREE.BufferAttribute(posArr, 3));
        const mat = new THREE.PointsMaterial({
            color: 0xdddddd, size: window.isMobile ? 4.0 : 8.0, map: window.textureKhoi,
            transparent: true, opacity: 0.6, blending: THREE.NormalBlending, depthWrite: false
        });
        const pts = new THREE.Points(geo, mat); scene.add(pts);
        hieuUngSmoker.push({ system: pts, velocities: vels, life: 15, upVector: new THREE.Vector3(0, 1, 0) });
    }

    function taoVatTheSmoker(tenFile, scaleSize) {
    ...
    function taoLocXoayKhoi(banKinh, chieuCao) {
        const group = new THREE.Group();
        ...
        for(let i=0; i<soLuong; i++) {
            ...
            let r = Math.random() * banKinh * (0.2 + 0.8 * (h / chieuCao)); 
        ...
        const mat = new THREE.PointsMaterial({
            color: 0xffffff, size: window.isMobile ? 5.0 : 10.0, map: window.textureKhoi,
            transparent: true, opacity: 0.6, blending: THREE.NormalBlending, depthWrite: false
        });
    ...
    // ==========================================
    window.tungComboSmoker = function (phim, isRemote = false, remoteGoc = null, remoteDich = null, remoteHuong = null, casterId = null, weaponUrl = null) {
        ...
        if (isRemote === false) {
            ...
            if (window.henGioTatMuaSmoker) clearTimeout(window.henGioTatMuaSmoker);
            window.henGioTatMuaSmoker = setTimeout(() => { window.dangMuaChieu = false; }, 1500);
        } else {
        ...
        if (isRemote) {
            ...
            let targetRadar = window.layMucTieuGanNhatSmoker(viTriGocToTam);
            ...
            if (window.room && window.room.localParticipant) {
                window.room.localParticipant.publishData(new TextEncoder().encode(JSON.stringify({
                    type: 'TUNG_CHIEU', skillType: phim, className: 'Smoker', 
        ...
        if (loaiChieu === 'Q') {
            ...
            setTimeout(() => {
                ...
                let tayTrai = timXuong(curNvc, ['LHand_Palm_053', 'LHand', 'mixamorigLeftHand']);
                let diemBan = curNvc.position.clone().add(cUp.clone().multiplyScalar(3.5));
                if (tayTrai) tayTrai.getWorldPosition(diemBan);

                const banTay = taoVatTheSmoker('bantaysmoker', 2.5); 
                banTay.position.copy(diemBan).add(cDir.clone().multiplyScalar(1.0));
                banTay.up.copy(cUp); banTay.lookAt(mucTieu); scene.add(banTay);
                
                kyNangSmoker.push({
                    mesh: banTay, type: 'BAY_THANG', speed: 8.0, life: 80, 
            ...
            setTimeout(() => {
                ...
                let tayPhai = timXuong(curNvc, ['RHand_Weapon_066', 'RHand_Palm_064', 'RHand_Palm_068', 'RHand', 'mixamorigRightHand']);
                let diemBan = curNvc.position.clone().add(cUp.clone().multiplyScalar(3.5));
                if (tayPhai) tayPhai.getWorldPosition(diemBan);

                const vuKhi = taoVatTheSmoker('vukhismoker', 3.0); 
                vuKhi.position.copy(diemBan).add(cDir.clone().multiplyScalar(1.0));
                vuKhi.up.copy(cUp); vuKhi.lookAt(mucTieu); scene.add(vuKhi);
                
                kyNangSmoker.push({
                    mesh: vuKhi, type: 'BAY_THANG', speed: 9.0, life: 80, 
        ...
        // =====================================
            ...
            setTimeout(() => {
                ...
                kyNangSmoker.push({ 
                    mesh: pivotGroup, swordMesh: vuKhiGiga, speed: 0, life: 200, ticks: 0, 
                    type: 'F_CHOP', targetPos: mucTieu.clone(), damage: dameGoc * 1.0, isRemote: isRemote, noBanKinh: 25, upVector: curUp.clone()
                });
            }, 300); 
        }
        ...
        // =====================================
            ...
            setTimeout(() => {
                ...
                let tayPhai = timXuong(curNvc, ['RHand_Weapon_066', 'RHand_Palm_064', 'RHand', 'mixamorigRightHand']);
                let diemBan = curNvc.position.clone().add(cUp.clone().multiplyScalar(2.0));
                if (tayPhai) tayPhai.getWorldPosition(diemBan);

                const locXoayR = taoLocXoayKhoi(10, 12);
        ...
        // =====================================
            ...
            setTimeout(() => {
                ...
                const locXoayF = taoLocXoayKhoi(50, 100);
    ...
    // ==========================================
    window.updateCombatSmoker = function () {
        for (let i = kyNangSmoker.length - 1; i >= 0; i--) {
            let s = kyNangSmoker[i]; s.life--;
            ...
            if (s.type === 'BAY_THANG') {
                if (s.targetPos) {
                    if (!s.isRemote) {
                        let objMoi = window.layMucTieuGanNhatSmoker(s.mesh.position);
                    ...
                    if (!isNaN(huongBay.x)) {
                        ...
                        if (typeof taoDuoiKhoi === 'function') taoDuoiKhoi(s.mesh.position, huongBay.clone().negate(), s.speed);
                    }
                } else {
                ...
                if (s.swordMesh) {
                    ...
                    if (s.ticks > 35 || s.life <= 5) {
                        taoHieuUngNoKhoi(s.targetPos, true, s.upVector); 
                        if (s.isRemote === false) gaySatThuongSmoker(s.targetPos, s.damage, s.noBanKinh);
                        else { 
                ...
                if (s.life % 10 === 0) {
                    let tickDame = s.damage * 0.15; 
                    if (s.isRemote === false) gaySatThuongSmoker(s.mesh.position, tickDame, s.noBanKinh);
                    ...
                    taoHieuUngNoKhoi(s.mesh.position, false, s.upVector);
                }

                if (s.life <= 0) {
                    if (s.isRemote === false) gaySatThuongSmoker(s.mesh.position, s.damage * 0.5, s.noBanKinh);
                    ...
                    taoHieuUngNoKhoi(s.mesh.position, true, s.upVector);
                    s.life = 0;
                }
            }
            ...
            if (s.life <= 0) {
                ...
                kyNangSmoker.splice(i, 1);
            }
        }

        for (let i = hieuUngSmoker.length - 1; i >= 0; i--) {
            ...
            for (let j = 0; j < posArr.length / 3; j++) {
                ...
                h.velocities[j].multiplyScalar(0.9); h.velocities[j].add(driftVec); 
            }
            ...
            if (h.life <= 0) {
                ...
                hieuUngSmoker.splice(i, 1);
            }
        }

        for (let i = danhSachSoBaySmoker.length - 1; i >= 0; i--) {
            let it = danhSachSoBaySmoker[i]; it.offsetY += 0.05; it.life--;
            ...
            if (it.life <= 0) { 
                ...
                danhSachSoBaySmoker.splice(i, 1); window.tongSoChuNoi_Smoker--; 
            }
        }
    };
    setInterval(window.updateCombatSmoker, 30);

    // ==========================================
    // 5. KHỞI TẠO & ĐĂNG KÝ HỆ PHÁI
    // ==========================================
    if (window.SCRIPT_PHAI_CUA_TOI && window.SCRIPT_PHAI_CUA_TOI.toLowerCase().includes('smoker')) {
        window.HePhaiHienTai = {
            tenPhai: "Thợ Săn Trắng Smoker",
            khoiTao: function () {
                console.log("🌪️ Khởi động Moku Moku no Mi chuẩn AAA!");
                if (typeof window.taiHoacNhanBanAsset === 'function') {
                    window.taiHoacNhanBanAsset('uploads/anims/bantaysmoker.glb', () => {});
                    window.taiHoacNhanBanAsset('uploads/anims/vukhismoker.glb', () => {});
                }
            ...
            tungChieu: window.tungComboSmoker,
            capNhat: function () { }
        };
...
// =========================================================================
window.tungCombosmoker = window.tungComboSmoker;
window.tungComboSmoker = window.tungComboSmoker;